...
- **Acciones de fila con `confirm()`:** el diálogo se abre desde el navegador, así que el callback de confirmación solo ejecuta un método que declare como `wireMethod` alguna acción con `confirm()` en `columns()`, que sea público y que reciba un único argumento escalar (la clave de la fila). No amplía lo que ya se podía llamar: sin `confirm()`, ese mismo método público se invoca con `wire:click`. Como con cualquier método público de Livewire, la autorización sobre el registro va dentro del método.
